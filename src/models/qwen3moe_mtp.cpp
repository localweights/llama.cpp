...
void llama_model_qwen3moe_mtp::load_arch_hparams(llama_model_loader & ml) {
    ml.get_key(LLM_KV_EXPERT_FEED_FORWARD_LENGTH,    hparams.n_ff_exp,      false);
    ml.get_key(LLM_KV_ATTENTION_LAYERNORM_RMS_EPS,   hparams.f_norm_rms_eps);

    ml.get_key(LLM_KV_NEXTN_PREDICT_LAYERS, hparams.nextn_predict_layers, false);
    GGML_ASSERT(hparams.nextn_predict_layers > 0  && "QWEN3MOE_MTP requires nextn_predict_layers > 0");
    GGML_ASSERT(hparams.nextn_predict_layers <= hparams.n_layer);
    GGML_ASSERT(hparams.n_expert > 0 && "QWEN3MOE_MTP requires n_expert > 0");
...
void llama_model_qwen3moe_mtp::load_arch_tensors(llama_model_loader &) {
    ...
    for (int i = 0; i < n_layer; ++i) {
        if (static_cast<uint32_t>(i) < n_main) {
            continue;  // trunk layer — owned by the sibling QWEN3MOE model
        }
        ...
        // Routed experts only (qwen3moe has no shared expert).
...
std::unique_ptr<llm_graph_context> llama_model_qwen3moe_mtp::build_arch_graph(const llm_graph_params & params) const {
    return std::make_unique<graph>(*this, params);
}

// LLM_ARCH_QWEN3MOE_MTP draft head for Qwen3-Coder-30B (plain GQA + plain RoPE + routed-only MoE)
llama_model_qwen3moe_mtp::graph::graph(const llama_model & model, const llm_graph_params & params)
    : llm_graph_context(params) {
    GGML_ASSERT(hparams.nextn_predict_layers > 0  && "QWEN3MOE_MTP requires nextn_predict_layers > 0");
    GGML_ASSERT(hparams.nextn_predict_layers == 1 && "QWEN3MOE_MTP currently only supports a single MTP block");
    ...
    ggml_tensor * head_norm_w = layer.nextn.shared_head_norm
            ...
            : model.output_norm;
    GGML_ASSERT(head_norm_w && "QWEN3MOE_MTP: missing both nextn.shared_head_norm and output_norm");
    cur = build_norm(cur, head_norm_w, nullptr, LLM_NORM_RMS, -1);
    cb(cur, "mtp_shared_head_norm", -1);

    ggml_tensor * head_w = layer.nextn.shared_head_head ? layer.nextn.shared_head_head : model.output;
    GGML_ASSERT(head_w && "QWEN3MOE_MTP: missing LM head (nextn.shared_head_head or model.output)");
